...
std::shared_ptr<Mat44_t> tracking_module::feed_frame(data::frame curr_frm) {
    ...
    if (curr_frm_.pose_is_valid()) {
          ...
          Mat33_t rot  = curr_frm_.get_pose_wc().block<3, 3>(0, 0);
          Vec3_t ea = rot.eulerAngles(2, 1, 0);
          out.open("/Users/kerianyousfi/lib2/stella_vslam_examples/build/4.txt", std::ios::app);
          out << ea[1]; out << " "; out << ea[2]; out << "\n";
          for (const auto& x: ea) std::cout << x*(M_PI/180) << " ";
          std::cout << std::endl;
          ...
          std::cout << "SUPPOSED 44:" << typeid((*cam_pose_wc).block<3, 3>(0, 0)).name() << std::endl;
      }
