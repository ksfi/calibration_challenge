...
cv::Mat get_mask(const cv::Mat& frame, Inference& inf)
{        
  std::vector<Detection> output = inf.runInference(frame);
  int detections = output.size();

  cv::Mat mask = cv::Mat(frame.rows, frame.cols, CV_8UC1, 255);
  for (int i = 0; i < detections; ++i)
  {
      ...
      cv::rectangle(mask, cv::Point(box.x, box.y), cv::Point(box.x+box.width, box.y+box.height), cv::Scalar(0), 1);
  }
  cv::rectangle(mask, cv::Point(0, frame.rows-300), cv::Point(frame.cols, frame.rows), cv::Scalar(0), 1);
//   cv::rectangle(mask, cv::Point(frame.cols/2-200, frame.rows/2-100), cv::Point(frame.cols/2+200, frame.rows/2+100), cv::Scalar(0), 1);
  cv::rectangle(mask, cv::Point(2*frame.cols/3-200, 0), cv::Point(frame.cols, frame.rows), cv::Scalar(0), 1);
  return mask;
}
...
//     const cv::Mat mask = mask_img_path.empty() ? cv::Mat{} : cv::imread(mask_img_path, cv::IMREAD_GRAYSCALE);
    ...
    unsigned int num_frame = 0, num_frame_prev = 0;
...
//             if (num_frame > 70) break;
            if (!frame.empty() && (num_frame % frame_skip == 0)) {
                // input the current frame and estimate the camera pose
                if (num_frame-num_frame_prev > 1) std::cout << "\n\n\nBBELLEK\n\n\n";
                std::cout << num_frame << std::endl;
                cv::Mat mask = get_mask(frame, inf);
